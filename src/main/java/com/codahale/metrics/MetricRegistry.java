...
import org.perf.agent.metrics.SQLTimer;
...
public class MetricRegistry implements MetricSet {
  ...
  public SQLTimer sqlTimer(String hash, String sql) {
    final Metric metric = getMetrics().get(hash);
    if (SQLTimer.class.isInstance(metric)) {
      return (SQLTimer) metric;
    } else if (metric == null) {
      try {
        return register(hash, new SQLTimer(sql));
      } catch (IllegalArgumentException e) { // NOSONAR
        final Metric added = getMetrics().get(hash);
        if (SQLTimer.class.isInstance(added)) {
          return (SQLTimer) added;
        }
      }
    }
    throw new IllegalArgumentException(hash + " is already used for a different type of metric");
  }
  ...
  public SortedMap<String, SQLTimer> getSQLTimers(MetricFilter filter) {
    return getMetrics(SQLTimer.class, filter);
  }
  ...
  public SortedMap<String, Timer> getTimers(MetricFilter filter) {
    SortedMap<String, Timer> defaultTimers = getMetrics(Timer.class, filter);
    SortedMap<String, SQLTimer> sqlTimers = getSQLTimers(filter);

    SortedMap<String, Timer> result = new TreeMap<String, Timer>();

    result.putAll(defaultTimers);
    result.putAll(sqlTimers);
